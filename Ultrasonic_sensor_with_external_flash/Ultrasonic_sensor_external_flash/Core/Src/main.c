...
int main(void)
{
	...
	while (1)
	{
		...
		Flash_Write(FLASH_ADDR, data, 4);
		HAL_Delay(50);

		uint8_t readback[4];
		Flash_Read(FLASH_ADDR, readback, 4);
		uint32_t stored = (readback[0] << 24) | (readback[1] << 16) | (readback[2] << 8) | readback[3];
